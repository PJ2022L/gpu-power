...
> 在 H800 上预测 GEMM、FlashMLA、FlashAttention v3 三类算子的性能与功耗；通过 microbenchmark 测出构建这些算子的 SASS 指令类功耗；最终让算子功耗预测值与实测值误差控制在 15% 以内。
...
| Python/PyTorch harness 规范在哪里？ | `02_tools/python_torch/README.md` | Python 调算子、shape/dtype 参数、Docker 默认 Python、日志字段。 |
...
| GEMM 算子资料在哪里？ | `03_operators/gemm/README.md` | GEMM shapes、实现来源、tensor core SASS、15% 功耗验证目标。 |
| FlashMLA 资料在哪里？ | `03_operators/flashmla/README.md` | FlashMLA decode/prefill、KV/cache、memory/SASS 关注点。 |
| FlashAttention v3 在哪里？ | `03_operators/flashattention_v3/README.md` | 记录实现、shape、kernel、SASS 和功耗验证。v2/v4 为后续扩展，当前不在 scope。 |
...
| `03_operators/` | GEMM、FlashMLA、FlashAttention v3 算子级资料（v2/v4 为后续扩展） | 不放通用工具教程，不放硬件白皮书原文。 |
...
- operator-driven：最终预测对象是 GEMM、FlashMLA、FlashAttention v3；
...
- validated：以算子实测功耗为 ground truth，目标误差不超过 15%。

## 项目约定

- H800 SXM/HGX 是第一目标；B200/Blackwell 暂作后续扩展。
- Python 脚本使用 H800 Docker 容器内默认 `python`，不要求额外环境激活。
...
- FlashAttention v2/v4 目录已建立，但当前不在 scope 内，作为后续扩展保留。
