...
- the experiment runs from the approved H800 runtime session,
...
The runtime environment may expose mounted data, GPU devices, and shared host resources. Treat these as production resources:

- do not manage the surrounding runtime lifecycle from this repository,
- do not remove or overwrite mounted data, `.agents/library/`, or raw experiment outputs,
- do not kill unrelated GPU processes,
- do not modify driver, CUDA, device, MIG, ECC, or system package state,
- do not install global packages during a measurement run,
- do not leak private environment variables, credentials, or host paths into public reports,
- do not change GPU clocks or power policy outside the phase protocol and logs.
...
Every Phase 08 microbenchmark power report and Phase 12 validation report must include:
