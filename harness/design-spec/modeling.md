...
This document defines the first H800 Wattchmen-style model boundary. It is a design contract for Phase 10 fitting and Phase 12 prediction, not a record of one experiment.

## Sources

- Wattchmen paper: `.agents/library/papers/Wattchmen-Watching the Wattchers.pdf`, especially Section 3.3.1 and Section 6.
- Knowledge summary: `.agents/knowledge/POWER_MODELING_METHODS.md`.
- Measurement protocol: `.agents/knowledge/POWER_MEASUREMENT_PROTOCOL.md`.
- Power environment spec: `harness/design-spec/power_measurement_environment.md`.
...
V1 treats non-negativity as a hard validity rule:

- measured active-window power and energy must be `>= 0`,
- `P_const_idle >= 0`,
- `P_static_full_activity = P_active_noop_full_sm - P_const_idle >= 0`,
- `E_dynamic_measured >= 0` after baseline subtraction,
- every fitted SASS or SASS-group energy coefficient must be `>= 0`,
- `P_dynamic_pred >= 0` and `P_operator_pred >= P_const_idle + P_static_full_activity`.

If a value violates these constraints, do not silently clamp it into the training set. Quarantine the row or model artifact, record the reason in the phase report, and inspect baseline quality, time-window alignment, repeatability, and benchmark saturation. Clamping is allowed only for explicitly labeled diagnostic plots, never for accepted fitting or validation artifacts.
...
Baseline collection is an explicit precondition for Phase 10. Use `scripts/collect_static_power_baselines.sh` in Phase 02 before fitting:

```bash
scripts/collect_static_power_baselines.sh configs/power/nvml_policy.yaml idle
scripts/collect_static_power_baselines.sh configs/power/nvml_policy.yaml empty_kernel_active -- <active-no-op-or-nanosleep-command>
```

Required raw artifacts:

```text
experiments/static_power/raw/<idle_baseline_id>/
  metadata.yaml
  power_trace.csv
  summary.yaml
  repeatability.yaml
experiments/static_power/raw/<active_noop_baseline_id>/
  metadata.yaml
  power_trace.csv
  summary.yaml
  repeatability.yaml
...
Phase 10 must record which baseline IDs were used to derive `P_const_idle` and `P_static_full_activity`.
...
- reject negative dynamic RHS rows unless they are explicitly quarantined,
- reject negative fitted coefficients,
...
Phase 03-05 provide operator kernel list, SASS counts, and memory/cache behavior. Phase 11 separately measures `T_operator` and operator power/energy in non-profiled runs.
...
- Phase 10 residual is low but Phase 12 operator error remains above 15%,
...
Any V2 change must be introduced through `harness/exec-plans/<xx+1>-plan.md`, evaluated as a full `xx-exp`, and recorded in `QUALITY.md`.
